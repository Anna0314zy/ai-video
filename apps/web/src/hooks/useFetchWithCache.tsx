...
const useFetchWithCache = (axiosFunction: any, refreshInterval: any, storageType = 'session') => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const cacheKey = `${import.meta.env.VITE_STORAGE_QINIU_KEY}`
      const cacheTimeKey = `${import.meta.env.VITE_STORAGE_QINIU_TIME}`
      const cachedData = storage.getItem(cacheKey)
      const cachedTimestamp: any = storage.getItem(cacheTimeKey)
      ...
      try {
        ...
        storage.setItem(cacheTimeKey, currentTime)
