...
export default (props: IStoryboardVideo) => {
  ...
  const onFinish = ({ uploadOptions }: { uploadOptions: IUploadOptions }) => {
    console.log('onFinish', uploadOptions, uploadOptions.storageFullPath)
    api
      .importResourceFile({ shotId: currentShotId, originPath: uploadOptions.storageFullPath, type: currentSelectType })
      .then(() => {
        onChangeGetNewData()
      })
  }
        ...
        <div>
          {currentSelectType === 'image' ? (
            <img
              ...
              src={getQiniuObjectUrl(item.compressUrl || item.originUrl)}
              alt=''
            />
          ) : (
            <video controls style={{ width: 1000 }}>
              <source src={getQiniuObjectUrl(item.compressUrl || item.originUrl)} type='video/mp4' />
      ...
      footer: (
        <div style={{ float: 'right' }}>
          ...
          <Button
            type={'primary'}
            onClick={() => {
              downloadQiniuObjectFile(item.compressUrl, item.name)
              // downloadFromServer(
              //   cdnPath +
              //     item.compressUrl +
