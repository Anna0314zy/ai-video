        ...
        for (const [index, shot] of shotDrafts.entries()) {
          await tx.shot.create({
            data: {
              ...
              soundEffectResourceIds: shot.soundEffectResourceIds,
              status: shot.status,
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        '',
        '输出 JSON 格式：',
        '{"shots":[{"title":"镜头标题","content":"镜头内容/动作描述","duration":5,"camera":"景别/镜头语言","scene":"场景","characters":"角色","visualPrompt":"给图片生成模型的英文或中英混合画面提示词","videoPrompt":"给视频生成模型的运动提示词","narration":"旁白文本","soundEffects":"音效描述，例如海浪声、脚步声、水花声、笑声","backgroundMusic":"背景音乐描述，例如轻快童趣的国风配乐"}]}',
        '',
        '要求：',
        '1. visualPrompt 必须能直接作为图片生成模型入参，包含主体、场景、风格、构图、光线。',
        '2. content 面向用户编辑，中文描述清楚镜头动作和画面。',
        '3. narration 是画外旁白，可为空，但不要编造与剧本无关的信息。',
        '4. soundEffects 只写非语言音效，例如环境声、动作声、笑声、水花声；没有则留空。',
        '5. backgroundMusic 只写音乐氛围，例如轻快、紧张、温暖；没有则留空。',
        '6. 不要把标题、总时长、分隔线、终幕说明单独拆成镜头。',
        '7. 如果剧本已有编号镜头，优先按编号镜头解析。',
        '',
        '剧本正文：',
        script.content || '',
      ].join('\n'),
...
function normalizeStructuredShot(item: any, index: number, script: any) {
  ...
  return {
    ...
    soundEffects: normalizeNonEmptyString(item?.soundEffects || item?.sfx || item?.soundEffect) || '',
    backgroundMusic: normalizeNonEmptyString(item?.backgroundMusic || item?.bgm || item?.music) || '',
    soundEffectResourceIds: '',
    status: 'uncompleted',
    imageStatus: 'uncompleted',
    videoStatus: 'uncompleted',
