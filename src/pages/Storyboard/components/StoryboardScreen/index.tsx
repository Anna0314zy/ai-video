  ...
  return (
    <Layout className={Styles['storyboard-image']}>
      <Layout.Sider className='storyboard-image-step'>
        <button className='btn-step' data-actived={step === 1} onClick={()=>{ setStep(1) }}>
          <div className='btn-step-index'>1</div>
          <div className='btn-step-text'>图片</div>
        </button>
        <div className='step-divider'></div>
        {/* 图片没有生成，不能操作视频 */}
        <button className='btn-step' data-actived={step === 2} disabled={props.disabled} onClick={()=>{setStep(2)}}>
